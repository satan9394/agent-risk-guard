...
# 2026-09-10 跨平台测试补回归：git 文件形态漏拦 / git rm / perl-ruby one-liner
$out += ,@(Test-Cmd 'git checkout -- f' 'deny')
$out += ,@(Test-Cmd 'git restore f' 'deny')
$out += ,@(Test-Cmd 'git rm f' 'deny')
$out += ,@(Test-Cmd 'git restore --help' 'allow')
$out += ,@(Test-Cmd 'git checkout --help' 'allow')
$out += ,@(Test-Cmd "perl -e 'unlink x'" 'deny')
$out += ,@(Test-Cmd "ruby -e 'File.delete(""x"")'" 'deny')
