...
| P0 | OpenCode | `scripts/opencode/agent-risk-guard.ts` | 复制到 `plugins/`（V2 `@opencode/cli` >= 2.0 由该目录**自动发现**，无需改配置；V1 1.18.x 还需在 `opencode.json` 的 `plugin` 数组**显式注册** —— R17 实测：仅放 plugins/ 不生效。见 `references/opencode-wiring.md`） |
