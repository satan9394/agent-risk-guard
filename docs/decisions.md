...
| 2026-09-22 | @opencode | **OpenCode V2 双形态支持**：V2（`@opencode/cli` >= 2.0）不再接受配置里的 `.ts` 文件路径（`configured plugin path must be a directory`）、本地插件改为 `~/.config/opencode/plugins/` 目录自动发现，且插件入口要求 `default` 是 `{ id, setup }`；此前 asset 与 installer/doctor 都是 V1-only（V2 机器上重跑安装会把配置写回 V1 形态）。改动：① `assets/opencode/agent-risk-guard.ts` 改为 **V1+V2 双入口**（纯对象 `{ ...plugin, server }`，无顶层 plugin 包 import，V1 的 `tool()` 在 `server()` 内动态 import；V2 侧接 `shell` / `tool` / `permission` hook 与 JSON-Schema 版 `trash` 工具；16 类 policy 检测核心逐字节保留）；② `packages/installer/src/merge.ts` 新增 `detectOpencodeConfigMode` / `findOpencodeRiskGuardRef`，`mergeOpencodePlugins` 在 V2 形态下**不写配置**、只清理 V1 遗留的文件路径引用；③ `runtime-probe.ts` / `doctor.ts` 的 wiring 判定按形态分流（V1 看 `plugin` 引用、V2 看 `plugins/` 目录产物 + hash 完整性）；④ `packages/cli/src/commands.ts` merge 传入 `home`（两键都没有时用 `cli.json` 作 V2 旁证），uninstall 同时清理 `plugin` 与 `plugins` | ✅ ACCEPT | PR [#25](https://github.com/satan9394/agent-risk-guard/pull/25) · `502b2cf`。**本机 D3（真实 OpenCode 2.0.13）**：`opencode plugin list` 显示 `agent-risk-guard` 加载成功（此前为 `failed to load plugin`）、`Remove-Item` 被 `PERMANENT_DELETE_POWERSHELL` 实际拦截、`trash` 工具可用、`opencode mcp list` 8/8。**回归**：`packages/installer/test/*` 9/9、`lifecycle.e2e` + 新增 `opencode-v2-lifecycle.e2e`、`transaction` 10/10、`tests/e2e/{cli,cli-doctor-freshness,install-ux}` + `standalone.e2e` 33/33、`tests/adapter/*` 全通过（CI `test (22)`/`test (24)` 亦通过）。**V1 行为零变化**：既有 V1 配置用例全部保持通过；用 `git stash` 复原原始源码复跑 `cli-exit-codes.e2e.test.ts` 同样 FAIL，证明该失败源于既存的 agy home 隔离问题、非本次引入 | 未发布 |

| 2026-09-22 | @opencode | **`--home` 密闭性修复**：`AGENT_REGISTRY` 的绝对探针（`%LOCALAPPDATA%/agy/bin/agy.exe`、`%APPDATA%/Cursor/…`）始终按**进程真实环境**展开，于是 `doctor --home <假 home>` / `detect --home <假 home>` 会把本机真实已装的 Agent 报成「已装」—— 假 home 的 doctor 凭空多出 `FAIL agy`（`tests/e2e/cli-exit-codes.e2e.test.ts` 两个用例在本机红；CI 因无 `~/.gemini` 而绿，问题只在装有该 Agent 的机器上暴露）。修法：显式传入且**不同于真实 home** 时进入**密闭模式**，按该 home 推导 `%APPDATA%` / `%LOCALAPPDATA%`（Windows 默认布局 `<home>/AppData/Roaming|Local`）；真实 home 或不传 home 时行为完全不变 | ✅ ACCEPT | `packages/installer/src/discovery.ts`（`expandProbePath` 增 `hermetic` 参数 + `detectAgent` 判定）+ 新增 `packages/installer/test/discovery-home-isolation.test.ts`（4 例：绝对探针落在假 home、假 home 不继承真机 agy、home 相对探针仍生效、真实 home 不密闭）。**回归**：`packages/installer/test/*` + `tests/e2e/*` + `tests/release-hardening/*` + `tests/transaction/*` 共 **134 pass / 0 fail**，其中 `cli-exit-codes.e2e` 由 **16/18 → 18/18**（修复前以原始源码复现同为 16/18，因果确认）。**CodeQL 复核**：首版路径归一用 `/[\\/]+$/` 被 `js/polynomial-redos` 判高危新告警（与本仓库 R7c 同类），改为线性去尾分隔符后告警消除 | 未发布 |
